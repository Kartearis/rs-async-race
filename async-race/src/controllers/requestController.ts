...
export default class RequestController {
    ...
    async getCars(perPage: number | null = null, page: number | null = null): Promise<CarListData> {
        const params: Record<string, string> | { _page: string; _limit: string } =
    ...
    async getCar(id: number): Promise<CarData> {
        ...
        if (response.status === 404) throw new Error('Requested car not found');
    ...
    async toggleEngine(id: number, status: EngineStates): Promise<EngineData> {
        if (id === undefined || id <= 0) throw new Error('Incorrect id');
        const response: Response = await this.#makeRequest('/engine', HttpMethods.PATCH, {
            id: id.toString(),
            status: status,
        });
        if (response.status === 200) return response.json();
        if (response.status === 400) throw new Error(`Bad request with id ${id} and status ${status}`);
        if (response.status === 404) throw new Error('Requested car not found');
    ...
    async evaluateDriving(id: number): Promise<boolean> {
        ...
        switch (response.status) {
            ...
            case 404:
                throw new Error('Requested has not started before');
            case 429:
                throw new Error('Cannot evaluate the same car several times');
            default:
                throw new Error('While evaluating car run some error occured');
        }
    }
    ...
    async getWinner(id: number): Promise<WinnerData> {
        ...
        if (response.status === 200) return response.json();
        if (response.status === 404) throw new Error('Requested winner not found');
    ...
    async createWinner(data: WinnerData): Promise<WinnerData> {
        ...
        if (response.status === 201) return response.json();
        if (response.status === 500) throw new Error('Could not create a record in winner table: Duplicate id');
    ...
    async #makeRequest(
        ...
        try {
            response = await fetch(url, {
                ...
                body: [HttpMethods.PUT, HttpMethods.POST, HttpMethods.PATCH].includes(method)
                    ? JSON.stringify(body)
                    : undefined,
            });
        } catch (e) {
